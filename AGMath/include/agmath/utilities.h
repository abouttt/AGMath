...
namespace agm
{
	...
	inline constexpr float Lerp(float start, float end, float t)
	{
	...
	{
		float delta = DeltaAngle(start, end);
		return WrapAngle(start + delta * Clamp01(t));
	}

	inline constexpr float InverseLerp(float start, float end, float value)
	{
		if (IsNearlyEqual(start, end))
		{
			return value <= start ? 0.f : 1.f;
		}
